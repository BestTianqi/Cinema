...
"Canvas 弧形座位图：小厅 100 座（每排 10 座）、中厅 200 座（每排 20 座）、大厅 300 座（每排 30 座），三个厅均为 10 排。每个座位显示排号和座号，空座绿色、选中黄色、已售红色，座区采用紧凑排距，并以银幕中心为参照，让同排两侧座位略靠前、中间座位略靠后，形成轻微弧形。Canvas 同时承担绘制和命中检测，未使用任何第三方图表库。",
...
("六、关键算法与数据结构", [
"放映厅配置 halls 保存名称、每排座位数和固定 10 排；座位使用 row、col、id、sold、x、y 和 r 描述。",
"用户表 sc_users 按用户名保存密码、角色和会员状态；sc_session 保存当前登录用户；sc_orders 保存订单数组；sc_sold 按厅保存已售座位编号。",
"推荐算法的时间复杂度约为 O(排数×列数×票数)，最大厅 300 座，浏览器中可以即时完成。",
